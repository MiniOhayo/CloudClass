...
/**
 * 中文注释 ShareSDK 官网地址 ： http://www.mob.com </br> 1、这是用2.38版本的sharesdk，一定注意 </br>
 * 2、如果要咨询客服，请加企业QQ 4006852216 </br> 3、咨询客服时，请把问题描述清楚，最好附带错误信息截图 </br>
 * 4、一般问题，集成文档中都有，请先看看集成文档；减少客服压力，多谢合作 ^_^</br></br></br>
 * 
 * The password of demokey.keystore is 123456 ShareSDK Official Website ：
 * http://www.mob.com </br> 1、Be carefully, this sample use the version of 2.11
 * sharesdk </br> 2、If you want to ask for help，please add our QQ whose number
 * is 4006852216 </br> 3、Please describe detail of the question , if you have
 * the picture of the bugs or the bugs' log ,that is better </br> 4、Usually, the
 * answers of some normal questions is exist in our user guard pdf, please read
 * it more carefully,thanks ^_^
 */
public class ThirdPartyLogin extends FakeActivity implements OnClickListener,
	...
	private void popupMsgLogin() {
		...
		View dlgView = View.inflate(activity, R.layout.tpl_msg_login_dialog,
				null);
		final EditText etPhone = (EditText) dlgView.findViewById(R.id.et_phone);
		final EditText etVerifyCode = (EditText) dlgView
				.findViewById(R.id.et_verify_code);
		Button btnGetVerifyCode = (Button) dlgView
				.findViewById(R.id.btn_get_verify_code);
		Button btnSendVerifyCode = (Button) dlgView
				.findViewById(R.id.btn_send_verify_code);
		btnGetVerifyCode.setOnClickListener(new OnClickListener() {
			@Override
			public void onClick(View v) {
				String phone = etPhone.getText().toString();
				if (TextUtils.isEmpty(phone)) {
					Toast.makeText(activity, "请输入手机号码", Toast.LENGTH_SHORT)
							.show();
				} else {
		...
		btnSendVerifyCode.setOnClickListener(new OnClickListener() {
			...
			public void onClick(View v) {
				...
				if (TextUtils.isEmpty(verifyCode)) {
					Toast.makeText(activity, "请输入验证码", Toast.LENGTH_SHORT)
							.show();
				} else {
	...
	private void popupOthers() {
		...
		View dlgView = View.inflate(activity, R.layout.tpl_other_plat_dialog,
				null);
	...
	public boolean handleMessage(Message msg) {
		...
		case MSG_AUTH_CANCEL: {
			// 取消授权
			Toast.makeText(activity, R.string.auth_cancel, Toast.LENGTH_SHORT)
					.show();
		}
			break;
		case MSG_AUTH_ERROR: {
			// 授权失败
			Toast.makeText(activity, R.string.auth_error, Toast.LENGTH_SHORT)
					.show();
		}
			break;
		case MSG_AUTH_COMPLETE: {
			...
			if (signupListener != null && signupListener.onSignin(plat, res)) {
				finish();
				Toast.makeText(activity, R.string.auth_complete,
		...
		case MSG_SMSSDK_CALLBACK: {
			if (msg.arg2 == SMSSDK.RESULT_ERROR) {
				...
				case SMSSDK.EVENT_SUBMIT_VERIFICATION_CODE: {
					...
					Toast.makeText(activity, "提交验证码成功", Toast.LENGTH_SHORT)
							.show();
				...
				case SMSSDK.EVENT_GET_VERIFICATION_CODE: {
					Toast.makeText(activity, "验证码已经发送", Toast.LENGTH_SHORT)
							.show();
				}
					break;
				}
